		...
		if ( ! empty( $_POST['login'] ) && ! empty( $_POST['_wpnonce'] ) && wp_verify_nonce( $_POST['_wpnonce'], 'woocommerce-login' ) ) {
			...
			try {
				...
				if ( $validation_error->get_error_code() ) {
					return wc_add_notice('<strong>' . __( 'Error', 'woocommerce' ) . ':</strong> ' . $validation_error->get_error_message(), 'error');
				}

				if ( empty( $username ) ) {
					return wc_add_notice('<strong>' . __( 'Error', 'woocommerce' ) . ':</strong> ' . __( 'Username is required.', 'woocommerce', 'error' )) ;
				}

				if ( empty( $_POST['password'] ) ) {
					return wc_add_notice('<strong>' . __( 'Error', 'woocommerce' ) . ':</strong> ' . __( 'Password is required.', 'woocommerce', 'error' ));
				}
				...
				if ( is_email( $username ) && apply_filters( 'woocommerce_get_username_from_email', true ) ) {
					...
					if ( isset( $user->user_login ) ) {
						...
						return wc_add_notice('<strong>' . __( 'Error', 'woocommerce' ) . ':</strong> ' . __( 'A user could not be found with this email address.', 'woocommerce', 'error' ));
					}
